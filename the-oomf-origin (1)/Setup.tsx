        ...
        <div className={`fixed inset-0 z-[200] overflow-y-auto`}
          ...
          <div className="min-h-full flex items-center justify-center p-4 pb-32 md:pb-24">
            ...
            <div className="max-w-[900px] w-full bg-[#111] p-4 md:p-5 rotate-1 border-[6px] border-yellow-500 shadow-[0px_0px_30px_rgba(255,215,0,0.3)] text-center relative">
                ...
                <h1 className="font-comic text-5xl md:text-6xl text-white leading-none mb-1 tracking-wider inline-block mr-3" style={{textShadow: '3px 3px 0px #d946ef'}}>THE OOMF</h1>
                <h1 className="font-comic text-3xl md:text-4xl text-yellow-400 leading-none mb-4 tracking-wider inline-block bg-black px-2 border border-yellow-400">ORIGIN STORY</h1>
                
                <div className="flex flex-col md:flex-row gap-4 mb-4 text-left">
                    
                    {/* Left Column: Cast */}
                    <div className="flex-1 flex flex-col gap-2">
                        <div className="font-comic text-xl text-white border-b-2 border-yellow-500 mb-1 tracking-wide">1. ASSEMBLE THE TEAM</div>
                        
                        {/* ROLE SELECTOR */}
                        <div className="mb-2">
                            <p className="font-comic text-sm text-gray-400 mb-1 tracking-wide">SELECT YOUR AGENT</p>
                            <div className="grid grid-cols-2 gap-2">
                                {ROLES.map(role => (
                                    ...
                                    >
                                        <div className="font-comic font-bold text-sm uppercase tracking-wide">{role.name.split(' (')[0]}</div>
                        ...
                        <div className={`p-3 border-2 border-dashed ${props.hero ? 'border-green-500 bg-green-900/20' : 'border-gray-500 bg-gray-900'} transition-colors relative group`}>
                            <div className="flex justify-between items-center mb-1">
                                <p className="font-comic text-base uppercase font-bold text-yellow-400 tracking-wide">YOUR AVATAR ({currentRole.name.split(' (')[0]})</p>
                            ...
                            {props.hero ? (
                                ...
                                <label className="comic-btn bg-[#d946ef] text-white text-sm px-3 py-2 block w-full hover:bg-[#c026d3] cursor-pointer text-center tracking-wide">
                                    UPLOAD PHOTO
                                    <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && props.onHeroUpload(e.target.files[0])} />
                                </label>
                            )}
                        ...
                        <div className={`p-3 border-2 border-dashed ${props.friend ? 'border-green-500 bg-green-900/20' : 'border-gray-500 bg-gray-900'} transition-colors mt-1`}>
                            <div className="flex justify-between items-center mb-1">
                                <p className="font-comic text-base uppercase font-bold text-cyan-400 tracking-wide">CIPHER (THE MENTOR)</p>
                            ...
                            {props.friend ? (
                                ...
                                <label className="comic-btn bg-cyan-600 text-white text-sm px-3 py-2 block w-full hover:bg-cyan-500 cursor-pointer text-center tracking-wide">
                    ...
                    <div className="flex-1 flex flex-col gap-2">
                        <div className="font-comic text-xl text-white border-b-2 border-yellow-500 mb-1 tracking-wide">2. CONFIGURE REALITY</div>
                        
                        <div className="bg-gray-900 p-3 border-2 border-gray-700 h-full flex flex-col justify-between">
                            <div>
                                <div className="mb-4">
                                    <p className="font-comic text-sm mb-1 font-bold text-gray-300 tracking-wide">VISUAL STYLE</p>
                                    <select value={props.selectedGenre} onChange={(e) => props.onGenreChange(e.target.value)} className="w-full font-comic text-lg p-2 border border-gray-600 uppercase bg-black text-white cursor-pointer focus:outline-none focus:border-yellow-500 transition-all tracking-wide">
                                        {VISUAL_STYLES.map(g => <option key={g} value={g} className="text-white">{g}</option>)}
                                    </select>
                                </div>

                                <div className="mb-4">
                                    <p className="font-comic text-sm mb-1 font-bold text-gray-300 tracking-wide">LOCATION</p>
                                    <select value={props.selectedCity} onChange={(e) => props.onCityChange(e.target.value)} className="w-full font-comic text-lg p-2 border border-gray-600 uppercase bg-black text-white cursor-pointer focus:outline-none focus:border-yellow-500 transition-all tracking-wide">
                                        {CITIES.map(c => <option key={c} value={c} className="text-white">{c}</option>)}
                                    </select>
                                </div>

                                <div className="mb-4">
                                    <p className="font-comic text-sm mb-1 font-bold text-gray-300 tracking-wide">LANGUAGE</p>
                                    <select value={props.selectedLanguage} onChange={(e) => props.onLanguageChange(e.target.value)} className="w-full font-comic text-lg p-2 border border-gray-600 uppercase bg-black text-white cursor-pointer tracking-wide">
                                ...
                                <div className="bg-black/50 p-2 border border-gray-700">
                                    <p className="font-comic text-yellow-500 text-sm mb-1 tracking-wide">BRIEFING:</p>
                            ...
                            <label className="flex items-center gap-2 font-comic text-sm cursor-pointer text-gray-300 mt-2 p-1 hover:bg-gray-800 rounded border border-transparent hover:border-gray-600 transition-colors tracking-wide">
